...
void do_init()
{
	...
	for (i = 0; i < PAGE_SUM; i++)
	{
		...
		pageTable[i].count = cixu++;
		pageTable[i].Visited = rand()%4+1;
		/* 使用随机数设置该页的保护类型 */
		switch (rand() % 7)
		{
			case 0:
		...
		/* 设置该页对应的辅存地scanf("%d %d",&Addr,&req);址 */
...
void do_response()
{
	...
	/* 检查页面访问权限并处理访存请求 */
	if (ptr_memAccReq->bianhao!=ptr_pageTabIt->Visited) {printf("没有访问权限！");return;}
	switch (ptr_memAccReq->reqType)
	{
		case REQUEST_READ: //读请求
		{
			ptr_pageTabIt->count=cixu++;
...
int main(int argc, char* argv[])
{
	...
	while (TRUE)
	{
	    scanf("%d %d %d",&Addr,&req,&Num1);
	  //  printf("3\n");
		do_request(Addr,req,Num1);
	//ptr_memAccReq->virAddr=Addr;
